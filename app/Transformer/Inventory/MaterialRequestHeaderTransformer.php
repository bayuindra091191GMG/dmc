...
class MaterialRequestHeaderTransformer extends TransformerAbstract
{
    ...
    public function transform(MaterialRequestHeader $header){
        ...
        if($this->type === 'before_create'){
            $code = "<a href='/admin/material_requests/". $url. "/detil/" . $header->id. "' style='text-decoration: underline;' target='_blank'>". $header->code. "</a>";
            ...
            $action = "<a class='btn btn-xs btn-success' href='". $routeIssuedDocket . "' data-toggle='tooltip' data-placement='top'><i class='fa fa-check-square'></i> Proses Issued Docket </a>";
        }
        else{
